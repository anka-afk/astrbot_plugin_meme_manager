...
图床仅用于云端同步，可在**设置中心 → 存储与图床**中选择并配置：
...
- 自动收集增加中文跳过原因日志，方便排查图片未进入识别队列的问题（[#136](https://github.com/anka-afk/astrbot_plugin_meme_manager/issues/136)）。
- 普通图片按重复出现次数逐步提高识别采样概率，首次仅记录，第八次必定触发；平台明确标记的表情沿用单独抽样设置。
- 单条消息图片处理上限可设置为 1–10 张，默认 1 张；同一消息中的多张图片共用来源冷却机会。
