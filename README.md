...
在 WebUI 的「语义化」页面选择资源包后，可以按图片 SHA-256 去重生成描述、标签并建立本地索引；GIF 会按首帧、中间帧和尾帧理解完整动作。任务支持暂停、继续和失败重试；索引文件保存在 AstrBot 插件数据目录，不会随资源包导出。完成后在插件设置中打开 `semantic.enabled`，回复模型会通过 `search_memes` 获取少量候选，并用 `&&meme:候选ID&&` 精确选择图片。没有视觉模型、向量模型或索引时会自动回到原有分类模式，旧资源包无需添加语义文件即可继续使用。
