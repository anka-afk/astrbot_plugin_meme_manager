...
**v5.0.1 更新了默认提示词，升级后请重置默认模板。** 升级并重载插件后，进入 **设置中心 → 提示词模板 → 恢复默认模板 → 保存并应用**，载入新版前缀、后缀及附加模板。已有自定义内容请先复制备份，再按需合并回新版模板。

新增的“表情回复数量提示”默认关闭，可在同一页面按需开启；它提供约每 4 条助手回复配图 1 次的参考节奏，与“表情发送”中的硬上限独立。提示词只引导模型，实际频率会随语境变化。

从 v4.x 升级的用户请注意：v5.0.0 起旧版平铺配置和配置别名不再读取或迁移，加载时会清除废弃字段。升级前请备份插件配置及表情包数据，升级后检查设置并重新填写仅存在于旧配置中的选项；当前分组结构中的有效设置会保留。
