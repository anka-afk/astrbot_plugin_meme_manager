...
- ☁️ **图床模块重制**：更新 R2、WebDAV 和 StarDots，按内容及传输记录识别修改，普通同步保留同名冲突和目标端额外图片。
- ⚠️ **双向覆盖**：可选择以本地或云端为准，确认前展示预计写入和删除数量；传输失败或目录读取不完整时停止后续清理。
- ⏹️ **可见进度与取消**：查看当前文件、处理数量和失败详情，支持停止任务及刷新后继续查看，关闭网页也会在任务结束后释放占用。
- 🖼️ **可靠传输**：图片校验后原子替换，改进超时、有限重试、分类路径和分页处理；R2 无公开地址仍可凭证同步。
- 🔄 **升级提醒**：旧版文件名记录不再作为内容证据，首次出现“待确认”时请先检查差异，再选择覆盖方向；不同图包仍使用配置指定的远端目录。
