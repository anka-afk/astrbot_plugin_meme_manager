...
图床仅用于云端同步，可在**设置中心 → 存储与下载**中选择并配置：

- 兰空图床（Lsky Pro 开源版 2.x）
- Cloudflare R2
- WebDAV
- StarDots
...
在**模式**页选择原生分类、辅助模型分类或两种语义检索模式，再沿流程图查看使用规则、选图、回退与发送设置。点击流程中的配置项可在弹窗内修改；也可以从**使用规则、表情发送、自动收集、存储与下载、提示词模板、标签与匹配、导入与备份**等分类进入。配置修改后点击“保存并应用”，使用规则单独保存。
...
平台明确标记为表情的图片直接进入抽样与识别流程；普通图片或缺少平台分类的图片首次仅记录，第二次出现时有 45% 的识别采样概率，之后逐次提高，第八次必定触发。同一发送者在不同消息中重复发送也会计数；连续 7 天未出现的记录会过期。单条消息最多处理的图片数可设置为 1–10 张，默认 1 张；同一来源最短识别间隔限制的是不同消息的入队时间，不影响重复次数记录。
...
页面预览默认同时加载 2 张图片，可在 AstrBot 插件配置的「页面预览」或插件设置中心的「存储与下载 → 页面预览」调整为 1–8 张，保存后刷新页面生效。并发限制按每个打开的插件页面计算。

已浏览的图片会使用浏览器私有缓存，文件未变化时重新进入页面可减少重复下载。替换文件后重新加载图库会使用新的图片版本；浏览器清理缓存或强制刷新时可能重新下载。此功能控制加载并发和重复流量，不限制服务器总带宽。
...
- 设置中心新增四种模式的完整流程图；点击配置项可在弹窗中直接修改，原有分类入口保留。
- 资源广场按上游版本显示可更新表情包，支持逐张预览并选择新增、修改和删除内容，以及多种更新策略。
- 优化表情包管理的拖拽与批量操作，支持跨表情包移动并处理目标分类冲突。
