async function initApp() {
  ...
  const libraryLoadState = document.getElementById("library-load-state");
  const libraryLoadMessage = document.getElementById("library-load-message");
  const libraryRetryBtn = document.getElementById("library-retry-btn");
  ...
  const retryManagePacksBtn = document.getElementById("retry-manage-packs-btn");
  ...
  function renderManagePackVectorStatus(packId, status, state = "ready") {
    ...
    if (taskBusy) {
      ...
      if (packVectorStatusText) {
        packVectorStatusText.textContent = `向量 ${
          indexDimension || configuredDimension || "未知"
        } 维`;
      }
    } else {
  ...
  async function performVectorRebuild(packId, knownStatus = null) {
    ...
    try {
      ...
      showToast(error?.message || String(error), "error", "向量重建失败", 5000);
      return false;
    } finally {
  ...
  function resetPackImportPreview({ keepResult = false } = {}) {
    ...
    if (packImportFileLabel)
      packImportFileLabel.textContent = "选择或拖入 zip 压缩包";
  ...
  function updateManagePackSemanticAppearance(packId) {
    ...
    const status = knownStatuses.includes(String(pack?.semantic_status || ""))
      ...
      : "none";
    if (packSemanticStatus) {
      packSemanticStatus.hidden = !pack || status === "none";
    }
  ...
  function buildCatalogPageUrl() {
    return withCurrentAuthParams("../catalog/index.html", {
      view: "catalog",
      asset_token: window.MemeUI.navAuthToken || null,
    }).toString();
  }
  ...
  async function loadManagePackSwitcher(preferredPackId = "") {
    ...
    if (retryManagePacksBtn) {
      retryManagePacksBtn.hidden = true;
    }
    try {
      const response = await apiGet("packs");
      const packs = Array.isArray(response?.packs) ? response.packs : [];
      ...
      managePackSelect.replaceChildren(new Option("资源包加载失败", ""));
      managePackSelect.disabled = true;
      if (retryManagePacksBtn) {
        retryManagePacksBtn.hidden = false;
      }
  ...
  async function switchManagePack() {
    ...
    if (
      !targetPackId ||
      targetPackId === activeManagePackId ||
      managePackSelect.disabled
    ) {
      return;
    }
    ...
    try {
      ...
      managePackSelect.value = previousActivePackId;
  ...
  function updateSemanticReviewToolbar() {
    ...
    semanticReviewStats
      .querySelectorAll("button[data-review-filter]")
      .forEach((button) => {
        ...
        button.classList.toggle(
          "active",
          filter === activeSemanticReviewFilter,
        );
        button.setAttribute(
          "aria-pressed",
          String(filter === activeSemanticReviewFilter),
        );
      });
  }
  ...
  async function fetchEmojis({ propagateError = false } = {}) {
    ...
    libraryLoadState.classList.remove("hidden", "error");
    libraryLoadMessage.textContent = "正在加载表情包…";
    libraryRetryBtn.classList.add("hidden");
    document
      .getElementById("emoji-categories")
    ...
    try {
      ...
      if (
        requestSequence !== libraryLoadSequence ||
      ...
      updateSelectionUI();
      libraryLoadState.classList.add("hidden");
    } catch (error) {
      if (
        requestSequence !== libraryLoadSequence ||
        requestedPackId !== activeManagePackId
      ...
      libraryRetryBtn.classList.remove("hidden");
      if (propagateError) throw error;
      return false;
    } finally {
      if (requestSequence === libraryLoadSequence) {
  ...
  function renderImageSemantic(semantic, { loading = false, error = "" } = {}) {
    ...
    imagePreviewCategoryReview?.classList.toggle("hidden", !showCategoryReview);
    ...
    if (imagePreviewReclassification) {
      const fromCategory = String(semantic?.reclassified_from_category || "");
    ...
    const showReviewActions = Boolean(
      canEdit && (semantic?.can_confirm_category || needsHumanReview),
    );
    ...
    if (imagePreviewVectorState) {
      const vectorLabels = {
        ...
        failed: "向量更新失败",
        running: "向量更新中",
        pending: manualModified ? "语义已保存，向量待更新" : "向量等待更新",
        cleared: "向量等待更新",
      };
    ...
    if (imagePreviewDuplicateNotice) {
      imagePreviewDuplicateNotice.textContent = duplicatePaths.length
        ? `检测到另外 ${
          ...
          } 张内容相同的图片。本次只修改当前路径，不会改动：${duplicatePaths.join(
            "、",
          )}`
        : "";
    ...
    imagePreviewSemanticActions?.classList.toggle("hidden", !showBottomActions);
    ...
    try {
      ...
      if (imagePreviewCategoryDecision) {
        if (
          ...
          ["return_original", "move_to_other"].includes(classificationAction) ||
      ...
      const originalCategory = String(proposal.original_category || "").trim();
      const selectedCategory = String(proposal.selected_category || "").trim();
  ...
  async function saveCurrentImageSemantic({ updateVector = false } = {}) {
    ...
    try {
      ...
      if (payload.target_category && !updateVector) {
        throw new Error("选择新分类后，请点击“保存、移动并更新该图向量”。");
      }
      setImageSemanticSaving(
        true,
        activeButton,
  ...
  async function refreshUi({
    ...
    if (emojis) {
      await Promise.all([
        fetchEmojis({ propagateError: true }),
        refreshManagePackSummaries(),
      ]);
    }
    if (syncStatus) {
  ...
  function updatePanelToggleState() {
    ...
    if (leftPanel) leftPanel.inert = !consoleVisible;
    if (directoryPanel) directoryPanel.inert = !directoryVisible;
  }
  ...
  function updateDragHudPosition(clientX, clientY) {
    ...
    dragHud.style.transform = `translate3d(${Math.round(x)}px, ${Math.round(
      y,
    )}px, 0)`;
  }
  ...
  function bindEmojiInteractions(emojiItem, category, emoji) {
    ...
    emojiItem.addEventListener("keydown", (event) => {
      if (event.target !== emojiItem || event.repeat || event.isComposing)
        return;
      if (event.key === "Enter" || event.key === " ") {
        event.preventDefault();
        emojiItem.click();
  ...
  function displayCategories(emojiData, tagDescriptions) {
    ...
    const categoryEntries = Object.entries(emojiData || {})
      .map(([category, emojis]) => {
        const categoryMatches = `${category} ${
          tagDescriptions?.[category] || ""
        }`
          .toLocaleLowerCase()
          .includes(librarySearch);
        const source = (Array.isArray(emojis) ? emojis : []).filter(
          (emoji) =>
            !librarySearch ||
            categoryMatches ||
            String(emoji).toLocaleLowerCase().includes(librarySearch),
        );
      ...
      .filter(
        ([, emojis]) =>
          (!librarySearch && activeSemanticReviewFilter === "all") ||
          emojis.length,
      );
    ...
    if (!categoryEntries.length || totalEmojiCount === 0) {
      ...
      hint.innerHTML =
        activeSemanticReviewFilter === "all" && !librarySearch
          ? `
      ...
      `
          : `<p class="empty-pack-hint-title">当前筛选条件下没有图片</p>
             <p class="empty-pack-hint-meta">试试其他名称，或清除搜索和审核筛选。</p>
             <div class="empty-pack-hint-actions"><button id="clear-library-filters" type="button">清除筛选</button></div>`;
      container.appendChild(hint);
      document
        .getElementById("clear-library-filters")
    ...
    categoryEntries.forEach(([category, emojis]) => {
      ...
      if (Array.isArray(emojis)) {
        emojis.forEach((emoji) => {
          ...
          emojiItem.setAttribute("role", "group");
          emojiItem.setAttribute(
            "aria-label",
            `${category} / ${emoji}，按回车预览`,
          );
          ...
          if (semanticReviewAvailable && review) {
            ...
            semanticBadge.title = reclassificationStatus
          ...
          deleteBtn.innerHTML = "×";
          deleteBtn.setAttribute("aria-label", `删除 ${emoji}`);
          deleteBtn.onclick = (e) => {
            e.stopPropagation();
      ...
      if (activeSemanticReviewFilter === "all" && !librarySearch) {
  ...
  function updateSidebar(data, tagDescriptions) {
    ...
    for (const category in data) {
      if (!document.getElementById(`category-${category}`)) continue;
      const li = document.createElement("li");
      const a = document.createElement("a");
      a.href = "#category-" + category;
      a.textContent = category;
      a.title = tagDescriptions?.[category] || category;
      a.addEventListener("click", () => {
        for (const link of sidebarList.querySelectorAll("a"))
  ...
  function updateSelectionDecorations() {
    ...
    document.querySelectorAll(".category").forEach((categoryDiv) => {
      ...
      const visibleItems = [...categoryDiv.querySelectorAll(".emoji-item")];
      const totalCount = visibleItems.length;
      const selectedCount = visibleItems.filter((item) =>
        isEmojiSelected(category, item.dataset.emoji),
      ).length;
      const filtered =
        Boolean(librarySearch) || activeSemanticReviewFilter !== "all";
  ...
  if (contextMenuMoveBtn) {
    contextMenuMoveBtn.addEventListener("click", () => {
      const menuItems = dedupeEmojiItems(contextMenuState.items);
      closeBatchContextMenu();
  ...
  librarySearchInput.addEventListener("input", () => {
    librarySearch = librarySearchInput.value.trim().toLocaleLowerCase();
    displayCategories(latestEmojiData, latestTagDescriptions);
    updateSidebar(latestEmojiData, latestTagDescriptions);
  });
  libraryRetryBtn.addEventListener("click", () => {
    void fetchEmojis();
  });
  ...
  retryManagePacksBtn?.addEventListener("click", async () => {
    ...
    try {
      await loadManagePackSwitcher(activeManagePackId);
      if (managePackSelect.value) await fetchEmojis();
    } finally {
      retryManagePacksBtn.disabled = false;
    }
  });
  ...
  async function checkImgHostSyncStatus(showAlert = true) {
    ...
    try {
      ...
      if (typeof data.remote_total_bytes === "number") {
        ...
        remoteStorageText = `${formatBytes(
          data.remote_total_bytes_estimated,
        )}（本地估算）`;
      }
...
initApp().catch((error) => window.MemeUI.showPageError(error));
