async function initSemanticPage() {
  ...
  function setDialog(open, title = "", message = "", confirmText = "确定", showCancel = true) {
  ...
  function showDialog(title, message, options = {}) {
    return new Promise((resolve) => {
      ...
      setDialog(true, title, message, options.confirmText || "确定", options.showCancel !== false);
    });
  }
      ...
      return await Promise.race([
        ...
        new Promise((_, reject) => {
          timer = window.setTimeout(
            () => reject(new Error("AstrBot 页面桥接未就绪，请从 WebUI 入口重新打开")),
            8000,
          );
        }),
      ]);
  ...
  function updateButtonState() {
    buttons.forEach((button) => {
      ...
      const needsEmbedding = ["start", "retry", "index", "dimension", "force"].includes(action)
        || (action === "resume" && latestStatus?.task_mode !== "caption_only");
      const needsCompleteCaption = ["index", "dimension"].includes(action);
      const needsVision = ["start", "resume", "retry", "force"].includes(action)
        && !captionComplete;
      const allowedByTaskState = !latestStatus || ({
  ...
  function renderStatus(data) {
    ...
    if (data.concurrency && !concurrencyDirty) concurrencyInput.value = String(data.concurrency);
    ...
    const groupOpen = (title, defaultOpen) => previousOpen.has(title)
      ? previousOpen.get(title)
      : defaultOpen;
    let taskText = {
      idle: "空闲",
      running: "运行中",
    ...
    if (data.task_status === "running" && !data.worker_alive) taskText = "已中断（可继续）";
    if (data.task_status === "paused" && data.active_request_count) taskText = "正在中断请求";
    const phaseText = {
      captioning: "生成图片描述",
      indexing: "建立向量索引",
      finished: "已结束",
    ...
    renderMetricGroup("图片和描述", [
      ["文件", data.file_total], ["独立图片", data.unique_total],
      ["重复复用", data.reused_duplicate_files], ["描述完成", data.caption_done],
      ["描述失败", data.caption_failed],
    ], groupOpen("图片和描述", true));
    renderMetricGroup("向量处理", [
      ["向量完成", data.embedding_done], ["向量失败", data.embedding_failed],
      ["向量模型", data.embedding_provider_ready ? "已配置并可用" : "不可用"],
      ["实际向量模型", [data.embedding_provider_id, data.embedding_model].filter(Boolean).join(" / ") || "自动选择中"],
    ...
    renderMetricGroup("视觉模型和消耗", [
      ["视觉模型", data.vision_provider_ready ? "已配置并可用" : "不可用"],
      ["当前视觉模型", [data.vision_provider_id, data.vision_model].filter(Boolean).join(" / ") || "未选择"],
      ["视觉调用次数", data.vision_calls], ["输入 Token", data.token_usage_input],
      ["输出 Token", data.token_usage_output], ["消耗 Token", data.token_usage_total],
    ], groupOpen("视觉模型和消耗", false));
    renderMetricGroup("其他状态", [
      ["任务队列", ({
      ...
      ["其他资源包任务", Array.isArray(data.other_active_tasks) ? data.other_active_tasks.length : 0],
      ["语义查询", data.semantic_enabled ? (data.semantic_config_ready ? "已配置" : "未配置") : "未启用"],
  ...
  async function loadRecordImage(item, size = "preview") {
    ...
    const requestPromise = schedulePreviewRequest(() => apiGet("meme_image_data", {
      managed_pack_id: packSelect.value,
      category,
      filename,
      size,
    }), size === "original").then((data) => {
  ...
  function renderRecords(data) {
    ...
    recordsNext.disabled = recordsCurrentPage >= recordsTotalPages || requestRunning;
    if (!records.length) {
    ...
    records.forEach((item) => {
      ...
      previewButton.setAttribute("aria-label", `放大查看 ${item.relative_path || "表情包"}`);
      ...
      if (cachedPreview) {
        ...
        void loadRecordImage(item).then((dataUrl) => {
      ...
      previewButton.addEventListener("click", () => {
        const dataUrl = previewCache.get(`${previewKey(item)}:preview`) || previewImage.src || "";
  ...
  function withCurrentAuthParams(targetPath, extraParams = {}) {
    ...
    for (const [key, value] of currentParams.entries()) {
      if (key !== "asset_token" && !nextUrl.searchParams.has(key)) nextUrl.searchParams.set(key, value);
    }
  ...
  async function applySecureNavLinks(pageApi) {
    ...
    document.querySelectorAll("a[data-nav-target]").forEach((link) => {
      ...
      if (!target) return;
      link.href = withCurrentAuthParams(target, { asset_token: token || null }).toString();
    });
  }
  ...
  async function runAction(apiPost, name) {
    ...
    if (name === "force") {
      const confirmed = await confirmTwice({
        ...
        finalMessage: "已有图片描述将被覆盖，并产生新的模型调用消耗。确定执行吗？",
        confirmText: "进入二次确认",
    ...
    if (name === "clear") {
      const confirmed = await confirmTwice({
        title: "确认清空当前任务队列",
        message: "这会取消正在运行的语义任务并清空待处理队列；已完成描述和原图片会保留。",
        finalTitle: "最后确认：取消并清空任务",
        finalMessage: "正在运行的任务会立即取消，未完成队列会被清空。确定执行吗？",
        confirmText: "进入二次确认",
    ...
    if (name === "delete-all") {
      const confirmed = await confirmTwice({
        title: "确认删除全部语义化数据",
        message: "这会删除当前资源包的全部图片描述、标签、失败记录、任务状态和本机向量索引；原图片不会删除。",
        finalTitle: "最后确认：彻底删除语义化数据",
        finalMessage: "删除后无法恢复已有描述，重新语义化会再次产生模型调用消耗。确定删除吗？",
        confirmText: "进入二次确认",
    ...
    setBusy(true);
    showNotice(name === "pause" ? "正在中断本轮请求并恢复等待队列……" : "正在提交操作……");
    try {
      const body = {
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape" && !imagePreviewMask.classList.contains("hidden")) {
      closeImagePreview();
    }
  });
  ...
  buttons.forEach((button) => button.addEventListener("click", () => runAction(apiPost, button.dataset.action)));
  ...
  concurrencyInput.addEventListener("change", () => {
    const value = Math.max(1, Math.min(16, Number(concurrencyInput.value) || 1));
  ...
  try {
    ...
    window.setInterval(() => loadStatus(apiGet).catch((error) => reportError("自动刷新失败", error)), 3000);
  } catch (error) {
    setBusy(false);
